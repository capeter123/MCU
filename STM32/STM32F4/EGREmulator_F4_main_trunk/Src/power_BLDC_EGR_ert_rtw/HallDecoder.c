...
#define S_FUNCTION_NAME HallDecoder
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          1
/* Input Port  0 */
#define IN_PORT_0_NAME      Hallx
...
/* Output Port  0 */
#define OUT_PORT_0_NAME      egr_bldc_actual_phase
...
extern void HallDecoder_Outputs_wrapper(const boolean_T *Hallx,
                          boolean_T *egr_bldc_actual_phase);
...
{
    const boolean_T   *Hallx  = (const boolean_T*) ssGetInputPortSignal(S,0);
    boolean_T        *egr_bldc_actual_phase  = (boolean_T *)ssGetOutputPortRealSignal(S,0);

    HallDecoder_Outputs_wrapper(Hallx, egr_bldc_actual_phase);
}
