/*****************************************************************************
 ...
 | --------     ---------------------     -------------------------------------
 | Bus          Sabine B�Eherl            Vector Informatik GmbH
...
/* Prepare DAQ */
static CCP_BYTE ccpPrepareDaq( CCP_BYTE daq, CCP_BYTE last,
CCP_BYTE eventChannel, CCP_WORD prescaler)
{
...
#ifdef CCP_MASTER_MSB_FIRST
			tmp = *(CCP_DWORD*) p;
			*(CCP_DWORD*) d = (CCP_DWORD) INVERSE_DWORD_ORDER(tmp);
#else
...
#ifdef CCP_MASTER_MSB_FIRST
			tmp = *(CCP_WORD*) p;
			*(CCP_WORD*) d = (CCP_WORD) INVERSE_WORD_ORDER(tmp);
#else
...
void ccpDaq( CCP_BYTE eventChannel)
{
	...
	for (o = 0, q = 0; q < CCP_MAX_DAQ; o += CCP_MAX_ODT, q++)
	{
		...
		{
			if (!ccpSampleAndTransmitDtm(
					(o + j) | (ccp.DaqList[q].flags & DAQ_FLAG_OVERRUN), q, j))
			{
				ccp.DaqList[q].flags |= DAQ_FLAG_OVERRUN;
			}
			else
...
#define daqCANId   (*(CCP_DWORD*)&com[4])
#ifdef CCP_DAQ
...
#ifdef CCP_MASTER_MSB_FIRST
#ifdef CCP_INTEL
#define ssPrescaler ((CCP_WORD)com[6] << 8 | com[7])
#endif
#else
...
static void ccpPrintDaqList( CCP_BYTE daq )
{
	...
	for (j=0;j<CCP_MAX_ODT;j++)
	{
		for (i=0,f=1;i<=6;i++)
		{
			...
			{
				if (f)
				{	CCPPRINT(" PID %u: ",daq*CCP_MAX_ODT+j); f = 0;}
#ifdef CCP_ODT_ENTRY_SIZE
